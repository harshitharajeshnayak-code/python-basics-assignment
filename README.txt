# 🚀 CampusPe – GenAI Internship Assignment  

## 👤 Author  
**Name:** Harshitha rajeshnayak  
**Program:** CampusPe GenAI Internship  
**Assignment Type:** Programming Exercises  

---

## 📌 Project Overview  

This repository contains solutions to **20 Python programming exercises** completed as part of the CampusPe GenAI Internship assignment.  

Each program focuses on strengthening core programming concepts such as:

- Loops  
- Conditional statements  
- Functions  
- String manipulation  
- Mathematical logic  
- Input handling  
- Edge case management  

All programs are written in **Python** and are designed to be executed individually.

---

## 📝 Questions Attempted  

| Q.No | Program Title |
|------|--------------|
| Q1  | Personal Bio Card |
| Q2  | Simple Calculator |
| Q3  | String Manipulator |
| Q4  | Age Calculator |
| Q5  | Bill Splitter |
| Q6  | Grade Calculator |
| Q8  | Leap Year Checker |
| Q10 | Simple ATM Simulator |
| Q11 | Number Pattern Printer |
| Q12 | Multiplication Table Generator |
| Q13 | Sum and Average Calculator |
| Q14 | Factorial Calculator |
| Q15 | Prime Number Checker |
| Q16 | Number Guessing Game |
| Q17 | Palindrome Checker |
| Q18 | Calculator Functions |
| Q20 | Number System Functions |

---

## ⚙️ How to Run the Programs  

1. Make sure Python is installed on your system.
2. Navigate to the project directory.
3. Run any program individually using:

```bash
python filename.py
```

Example:

```bash
python Q1_personal_bio_card
```

---

## 🛠 Technologies Used  

- Python 3  
- Standard Library Functions  

---

## 💡 Key Learning Outcomes  

During the completion of these exercises, the following concepts were strengthened:

- Writing clean and structured Python code  
- Implementing loop logic effectively  
- Handling edge cases (negative numbers, zero values, etc.)  
- Working with string operations and built-in functions  
- Designing simple interactive CLI programs  

---

## 🚧 Challenges Faced  

- Implementing loop logic correctly  
- Managing edge cases (negative numbers, zero, boundary conditions)  
- Understanding and applying string operations effectively  

---

## 📈 Conclusion  


This assignment helped reinforce fundamental programming concepts and improved problem-solving skills using Python. Each exercise builds practical understanding that serves as a strong foundation for advanced development tasks.
